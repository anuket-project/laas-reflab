...
use dal::{new_client, web::*, AsEasyTransaction, DBTable, ExistingRow, FKey, ID};
...
#[debug_handler]
async fn get_many_users(Json(usernames): Json<Vec<String>>) -> Result<Json<Vec<User>>, WebError> {
    ...
    for u in usernames {
        let u = ipa
            .find_matching_user(u, true, false)
            .await
            .log_server_error("Failed to find user", true)?;
