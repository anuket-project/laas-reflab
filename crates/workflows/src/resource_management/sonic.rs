use common::prelude::tracing::{self};
...
impl SonicSwitch {
    pub fn pull_config(session: &mut Session) -> SonicConfig {
        static SONIC_CFGGEN: &str = "/usr/local/bin/sonic-cfggen -d --print-data";
        let mut channel = session.channel_session().unwrap();
        let mut cfg = String::new();

        channel
            .exec(SONIC_CFGGEN)
            .expect("Running sonic-cfggen failed");

        channel
            .read_to_string(&mut cfg)
            .expect("Failed to read config into string");

        let config: SonicConfig =
            serde_json::from_str(cfg.as_str()).expect("Failed to serialize SONiC config");
    ...
    pub fn push_config(&mut self) {
        ...
        let config_json = serde_json::to_string_pretty(&self.config)
            .expect("Failed to serialize config into JSON");
        ...
        file.write_all(config_json.as_bytes())
            .expect("Failed to write config to remote host");

        std::mem::drop(file);
        std::mem::drop(sftp);

        channel
            .exec("sudo mv ~/config_db.json /etc/sonic/")
            .expect("Failed to move config");
        ...
        channel
            .exec("sudo config reload --yes")
            .expect("Failed to run config load cmd");
    }
    ...
    */
    pub fn member_list(&self) -> impl Iterator<Item = (&String, String, String, &SonicVLANMember)> {
        self.config.VLAN_MEMBER.iter().map(|(name, member)| {
            ...
            let vlan_name = (*parsed_pair.first().unwrap()).to_owned();
    ...
    pub fn set_single_vlan(&mut self, vlan: i16, iface: &str, tagged: bool) {
        ...
        let vlan = self
            .config
            .VLAN
            .entry(vlan_name.clone())
            .or_insert(SonicVLAN {
                admin_status: "up".to_owned(),
                members: HashSet::new(),
        ...
        if vlan.vlan_id != vlan_id {
            panic!(
                "Improperly set up VLAN: {} had vlan ID {}",
        ...
        if !tagged {
            ...
            let maybe_previously_untagged_vlan =
                self.member_list()
                    .find(|(_, old_vlan_name, interface_name, member)| {
                        !vlan_name.eq(old_vlan_name)
                            && interface_name.eq(iface)
                            && member.tagging_mode == "untagged"
                    });

            if let Some((name, old_vlan_name, _, _)) = maybe_previously_untagged_vlan {
                to_delete = Some(name.to_owned());

                self.config.VLAN.entry(old_vlan_name).and_modify(|x| {
                    x.members.remove(iface);
                });
            }
        ...
        self.config.VLAN_MEMBER.insert(
            vlan_member_name,
            SonicVLANMember {
    ...
    */
    pub fn set_interface_vlans(
        &mut self,
        untagged_vlan: Option<i16>,
        tagged_vlans: Vec<i16>,
        iface: String,
    ) {
        let vlan_names = tagged_vlans
            .iter()
            .chain(untagged_vlan.iter())
            .map(|x| format!("Vlan{}", x))
            .collect::<Vec<String>>();

        // Get all VLANs on this interface
        let unmentioned_vlans = self
            .member_list()
    ...
    pub fn run_commands(&mut self, persistent: bool) -> Result<(), String> {
        ...
        for cmd in self.commands.iter() {
            ...
            channel
                ...
                .expect("Expected to write command as bytes to channel!");
            channel
                .write_all(b"\n")
        ...
        channel
            .send_eof()
            .expect("Expected to send eof to channel!");
        let mut info: String = String::default();
        channel
            .read_to_string(&mut info)
            .expect("Expected to read info!");
        self.commands.clear();
...
pub async fn sonic_run_network_task(ncfg: NetworkConfig) {
    ...
    let mut client = new_client().await.expect("Did not get a database client");
    ...
    for bg in ncfg.bondgroups.iter() {
        ...
        for member in bg.member_host_ports.iter() {
            ...
            if let Some(sp_key) = host_port.switchport.as_ref() {
                let switch_port = sp_key.get(&mut transaction).await.unwrap();

                let for_switch = &switch_port.for_switch;
                let switch = for_switch.get(&mut transaction).await.unwrap().into_inner();
                    ...
                    == *"SONiC"
                {
                    ...
                    sanic.set_interface_vlans(
                        native_vlan,
                        vlans.clone(),
                        adams_law(switch_port.name.clone()),
                    );
                }
            }
        }
...
fn adams_law(iface: String) -> String {
    match iface {
        _ if iface.contains("GigE") && iface.contains('b') => {
            ...
            nums.next();
            let e = i32::from_str_radix(nums.next().expect("Expected digit"), 10)
                .expect("Expected to find a digit");
            let b = i32::from_str_radix(nums.next().expect("Expected digit"), 10)
                .expect("Expected to find a digit");
...
mod test {
    ...
    use super::{adams_law, SonicConfig, SonicSwitch};
    ...
    fn adams_law_test(input: String, expected: String) {
        ...
        assert_eq!(
            expected, result,
            "Output is incorrect. Returned \"{}\" instead of \"{}\"",
            result, expected
        );
    }
    ...
    */
    fn get_dummy_switch() -> SonicSwitch {
        SonicSwitch {
            ...
            config: SonicConfig {
                ...
                .unwrap(),
                VLAN_MEMBER: serde_json::from_str(
                    r#"{
                    "Vlan3002|Ethernet0": {
    ...
    fn set_single_vlan_new_iface() {
        ...
        assert!(test_sw
            .config
            .VLAN_MEMBER
            .contains_key("Vlan3002|Ethernet0"));
    ...
    fn set_interface_vlans_on_existing_interface() {
        ...
        assert!(test_sw
            ...
            .VLAN_MEMBER
            .get("Vlan3002|Ethernet0")
            .is_some_and(|x| x.tagging_mode.eq("tagged")));
        assert!(test_sw
            .config
            .VLAN_MEMBER
            .get("Vlan99|Ethernet0")
            .is_some_and(|x| x.tagging_mode.eq("tagged")));
        assert!(test_sw
            .config
            .VLAN_MEMBER
            .get("Vlan98|Ethernet0")
            .is_some_and(|x| x.tagging_mode.eq("untagged")));
        assert!(test_sw
            .config
            .VLAN
            .get("Vlan3002")
            .is_some_and(|x| x.members.contains(iface)));
        assert!(test_sw
            .config
            .VLAN
            .get("Vlan98")
