...
pub async fn send_to_admins_email_template(env: &Env, notification: Notification) -> Result<(), anyhow::Error> {
    ...
    let email =  Message::builder()
            ...
            .to(Mailbox::new(None, Address::new(to_addr.username.clone(), to_addr.domain.clone()).unwrap()))
            .subject(notification.title.clone())
            .header(ContentType::TEXT_HTML)
            .body(
                render(&notification, config::RenderTarget::Email)
