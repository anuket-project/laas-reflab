...
async fn sync_vpn_for_user(user: &String, managed_groups: Vec<String>, ipa: &mut ipa::IPA, transaction: &mut EasyTransaction<'_>) -> Result<(Vec<String>, Vec<String>), anyhow::Error> {

        let active_groups: Vec<String> = ipa.group_find_user(user).await?.iter().filter(|&g| managed_groups.contains(g)).cloned().collect();
        let correct_groups: Vec<String> = correct_groups_for_user(transaction, user).await?.iter().filter(|&g| managed_groups.contains(g)).cloned().collect();
...
pub async fn single_vpn_sync_for_user(user: &String) -> Result<(Vec<String>, Vec<String>), anyhow::Error> {
    let mut client = new_client().await.expect("Expected to connect to db");
    let mut transaction = client
        .easy_transaction()
        .await
