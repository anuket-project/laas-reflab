...
use models::{
    dal::{new_client, AsEasyTransaction, DBTable, EasyTransaction, ExistingRow, FKey, Lookup},
    dashboard::{Aggregate, EasyLog, StatusSentiment},
    inventory::{Arch, BootTo, DataUnit, DataValue, Flavor, Host, HostPort, ImportFlavor, ImportHost, Lab}, macaddr::MacAddress,
};
...
use crate::{
    deploy_booking::{
        ...
        set_host_power_state::{self, HostConfig, TimeoutConfig},
...
impl AsyncRunnable for InspectHost {
    ...
    async fn run(&mut self, context: &Context) -> Result<Self::Output, TaskError> {
        ...
        let _ = channel.exec("for f in /sys/kernel/debug/i40e/*/command; do
        ...
        let arch = {
            ...
            channel.exec("lscpu | awk '$1 == \"Architecture:\" {print $2}'").expect("Expected to get host arch info");
            channel
        ...
        let import_host = ImportHost {
            ...
            ipmi_mac: MacAddress::parse_str(self.ipmi_mac.clone().as_str()).expect("Expected valid mac addr"),
        ...
        let cpu_count = {
            ...
            channel.exec("lscpu | awk '$1 == \"CPU(s):\" {print $2}'").expect("Expected to get host arch info");
            channel
                .read_to_string(&mut res)
        ...
        let ram = {
            ...
            channel
                ...
                match res.clone() {
                    s if s.contains('M') => {DataValue::from_decimal(&s.split('K').next().unwrap(), DataUnit::KiloBytes)},
                    s if s.contains('M') => {DataValue::from_decimal(&s.split('M').next().unwrap(), DataUnit::MegaBytes)},
                    s if s.contains('G') => {DataValue::from_decimal(&s.split('G').next().unwrap(), DataUnit::GigaBytes)},
                    s if s.contains('T') => {DataValue::from_decimal(&s.split('T').next().unwrap(), DataUnit::TeraBytes)},
        ...
        let root_size = {
            ...
            let disk_info : (&str, &str) = serde_json::from_str(&res).expect("Expected to deserialize disk info"); 
            match disk_info {
                (_, s) if s.contains('M') => {DataValue::from_decimal(&s.split('K').next().unwrap(), DataUnit::KiloBytes)},
                (_, s) if s.contains('M') => {DataValue::from_decimal(&s.split('M').next().unwrap(), DataUnit::MegaBytes)},
                (_, s) if s.contains('G') => {DataValue::from_decimal(&s.split('G').next().unwrap(), DataUnit::GigaBytes)},
                (_, s) if s.contains('T') => {DataValue::from_decimal(&s.split('T').next().unwrap(), DataUnit::TeraBytes)},
        ...
        let disk_size = {
            ...
            let disk_info : (&str, &str) = serde_json::from_str(&res).expect("Expected to deserialize disk info"); 
            match disk_info {
                (s, _) if s.contains('K') => {DataValue::from_decimal(&s.split('K').next().unwrap(), DataUnit::KiloBytes)},
                (s, _) if s.contains('M') => {DataValue::from_decimal(&s.split('M').next().unwrap(), DataUnit::MegaBytes)},
                (s, _) if s.contains('G') => {DataValue::from_decimal(&s.split('G').next().unwrap(), DataUnit::GigaBytes)},
                (s, _) if s.contains('T') => {DataValue::from_decimal(&s.split('T').next().unwrap(), DataUnit::TeraBytes)},
                (_,_) => DataValue::from_decimal("0", DataUnit::GigaBytes)
            }.expect("Expected to parse")
        };
        ...
        if Flavor::lookup(&mut transaction, vec![self.flavor.clone()]).await.is_err() {
